...
async function main() {
    ...
    const vertShaderSample =
        ...
        void main() {
            ...
            oTangent = normalize(vec3(uModelMatrix * vec4(1.0, 0.0, 0.0, 0.0)).xyz); //assume tangent line lies on the x axis of model
        }
        `;
    ...
    const fragShaderSample =
        ...
        void main() {
            ...
            //if (samplerExists == 1){
                ...
                //normVector = normVector * 2.0 -1.0;
                //vec3 T = normalize(oTangent);
                //vec3 N = normalize(oNormal);
                //vec3 B = cross(oNormal, oTangent);
                //mat3 TBN = mat3(vec3(T), vec3(B), vec3(N));
                //vec3 normal = normalize(TBN * normVector);
                //
            ...
            //Ambient term Ka * La
            vec3 ambient = ambientVal.rgb * mainLight.colour.rgb * mainLight.strength;

            //Diffuse term Ld * (N dot L)
            //multiply Kd later as it changes with texture
            vec3 lightDirection = normalize(mainLight.position - oFragPosition);
            float diff = max(dot(normal, lightDirection), 0.0);
            vec3 diffuse = diff * mainLight.colour.rgb * mainLight.strength;
            ...
            vec3 specular = specularVal.rgb * mainLight.colour.rgb * pow(HdotN, nVal);

            float dist = length(mainLight.position - oFragPosition);
            float attenuation = mainLight.strength / (1.0 + mainLight.linear * dist + mainLight.quadratic * (dist * dist));
            ...
            if (samplerExists == 1) {
                ...
                //
                vec3 Kd = mix(diffuseVal, textureColour.rgb, 0.7);
                diffuse = Kd.rgb * diffuse;
                //
...
function drawScene(gl, deltaTime, state) {
    ...
    //
    //gl.depthMask(false); //if run this models render in weird order so left out
    gl.enable(gl.BLEND);
